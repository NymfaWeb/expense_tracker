...
<template>
  <div class="space-y-8 relative">
    <div class="flex items-center justify-between">
      ...
      <Button
        @click="expenseStore.toggleAddExpenseModal()"
        variant="default"
        class="bg-[#2dd4bf] hover:bg-teal-500 text-white shadow-lg shadow-teal-500/20"
      >
        Add Expense
      </Button>
    </div>
    ...
    <div
      v-if="isPending || isPendingSummary"
    ...
    <template v-else>
      ...
      <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        <Card
          class="p-6 bg-card/40 backdrop-blur-sm border-white/10 hover:bg-card/60 transition-colors"
        >
          ...
          <div class="mt-2">
            ...
            <p class="text-xs text-muted-foreground mt-1 flex items-center gap-1">
              <span class="text-emerald-500 flex items-center"
                ><ArrowUpRight class="h-3 w-3 mr-1" /> +12%</span
              >
              from last month
            </p>
          </div>
          ...
          <div class="mt-2">
            ...
            <p class="text-xs text-muted-foreground mt-1 flex items-center gap-1">
              <span class="text-rose-500 flex items-center"
                ><ArrowUpRight class="h-3 w-3 mr-1" /> Dynamic</span
              >
              based on DB
            </p>
          </div>
        ...
        >
          <div class="flex items-center justify-between space-y-0 pb-2">
            ...
            <Activity class="h-4 w-4 text-[#2dd4bf]" />
          </div>
          <div class="mt-2">
      ...
      <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-7">
        <Card class="col-span-4 p-6 bg-card/40 backdrop-blur-sm border-white/10">
          ...
          <div
            class="h-[300px] w-full rounded-md bg-muted/20 flex items-center justify-center border border-dashed border-muted"
          >
        ...
        <Card
          class="col-span-3 p-6 bg-card/40 backdrop-blur-sm border-white/10 overflow-hidden flex flex-col"
        >
          <div class="flex flex-col space-y-1.5 pb-4">
            <h3 class="font-semibold leading-none tracking-tight">Recent Transactions</h3>
            <p class="text-sm text-muted-foreground">
              Your latest {{ recentTransactions.length }} expenses.
            </p>
          </div>
            ...
            >
              ...
              <div class="flex-1 space-y-1 min-w-0">
                ...
                <p class="text-xs text-muted-foreground truncate">
                  {{ expense.expenseDate }} • {{ expense.isBusiness ? 'Business' : 'Private' }}
                </p>
              </div>
              <div class="font-medium text-sm tabular-nums whitespace-nowrap">
